...
def main():
	try:
			...
			if st.session_state.login == False:
				...
				st.session_state.option = sac.menu([
					...
					sac.MenuItem('Lesson Assistant', icon='person-fill-gear', children=[
						sac.MenuItem('Lesson Generator', icon='pencil-square', disabled=is_function_disabled('Lesson Generator')),
						sac.MenuItem('Lesson Feedback', icon='chat-left-dots', disabled=is_function_disabled('Lesson Feedback')),
							]),
					sac.MenuItem('Learning Tools', icon='tools', children=[
						sac.MenuItem('Knowledge Map Generator', icon='diagram-3-fill', disabled=is_function_disabled('Knowledge Map Generator')),
						sac.MenuItem('Conversation Assistant', icon='people-fill', disabled=is_function_disabled('Conversation Assistant')),
							]),
					sac.MenuItem("Remarks Co-Pilot", icon='link-45deg', href='https://remarkscopilot.vercel.app', disabled=is_function_disabled('Remarks Copilot')),
					sac.MenuItem('Dialogic Agent', icon='robot', children=[
						sac.MenuItem('Chatbot', icon='chat-square-dots', disabled=is_function_disabled('Chatbot')),
		...
		#Lesson Assistant
			...
			if prompt:
				lesson_bot(prompt, st.session_state.lesson_generator, LESSON_COLLAB)	
					
		elif st.session_state.option == "Lesson Feedback":
			st.subheader(f":green[{st.session_state.option}]")
			prompt = lesson_commentator()
			if prompt:
				lesson_bot(prompt, st.session_state.lesson_feedback, LESSON_COMMENT)
