...
def main():
    try:
            ...
            if st.session_state.login == False:
                ...
                st.session_state.option = sac.menu([
                    sac.MenuItem('Home', icon='house', children=[
                        ...
                        sac.MenuItem('Class Dashboard', icon='clipboard-data',
                                     disabled=is_function_disabled('Class Dashboard')),
                    ]),
                    sac.MenuItem('Lesson Assistant', icon='person-fill-gear', children=[
                        sac.MenuItem('Lesson Collaborator', icon='pencil-square',
                                     disabled=is_function_disabled('Lesson Collaborator')),
                        sac.MenuItem('Lesson Commentator', icon='chat-left-dots',
                    ...
                    sac.MenuItem('Learning Tools', icon='tools', children=[
                        sac.MenuItem('Knowledge Map Generator', icon='diagram-3-fill',
                                     disabled=is_function_disabled('Mindmap Generator')),
                        sac.MenuItem('Conversation Assistant', icon='people-fill',
                                     disabled=is_function_disabled('Conversation feedback')),
                    ]),
                    sac.MenuItem('Dialogic Agent', icon='robot', children=[
                        sac.MenuItem('Chatbot', icon='chat-dots',
                    ...
                    sac.MenuItem('Remarks Co-Pilot', icon='link'),
                    ...
                    # Assuming these are always available
                    sac.MenuItem('OpenAI API Key', icon='key-fill'),
        ...
        remarks_copilot_link = st.empty()
        if st.session_state.option == "Remarks Co-Pilot":
            remarks_copilot_link.markdown(
                '[Go to Remarks Co-Pilot](https://remarkscopilot.vercel.app/)')
        ...
        if st.session_state.option == 'Users login':
            ...
            with placeholder2:
                ...
                with col2:
                    st.write(
                        "Please read.me before using or forking this application")
        ...
        # Dialogic Agent
            ...
            if st.session_state.api_key == False:
                sac.alert(message='**OpenAPI API Key not found, enter an API key for the chatbot to function**',
                          description=None, type='error', height=None, icon=True, closable=True, banner=True)
            ...
            col1, col3, col4, col5, col6, col7 = st.columns(
                [2, 1, 1, 1, 1, 1])  # column separation
            with col1:
                st.write(
                    f"Knowledge Base: :green[{st.session_state.current_model}]")
            ...
            # 		st.session_state.visuals = False
            with col3:
                clear = sac.switch(label='Clear Chat', value=False,
                                   align='start', position='left', size='default')
                if clear == True:
                    clear_session_states()
            with col4:
                mem = sac.switch(label='Enable Memory', value=True,
                                 align='start', position='left', size='default')
                if mem == True:
                    st.session_state.memoryless = False
                else:
            ...
            with col5:
                rating = sac.switch(label='Rate Response', value=True,
                                    align='start', position='left', size='default')
                if rating == True:
                    st.session_state.rating = True
                else:
            ...
            if st.session_state.vs:  # chatbot with knowledge base
                with col6:
                    raw_search = sac.switch(
                        label='Raw Search', value=False, align='start', position='left', size='default')
                with col7:
                    unload = sac.switch(
                        label='Unload KB', value=False, align='start', position='left', size='default')
                    if unload == True:
                        st.session_state.vs = False
                        st.session_state.current_model = "No KB loaded"
                        st.experimental_rerun()
                ...
                chat_bot_vectorstore_selection_interface(
                    st.session_state.user['id'], col6, col7)
        ...
        # Knowledge Base Tools
            ...
            st.subheader(f":green[{st.session_state.option}]")
            if st.session_state.api_key == False:
                sac.alert(message='**OpenAPI API Key not found, enter an API key to create the vectorstores**',
                          description=None, type='error', height=None, icon=True, closable=True, banner=True)
            options = sac.steps(
                items=[
                ...
                steps_options = sac.steps(
                    items=[
                        ...
                        sac.StepsItem(title='step 3',
                                      description='Change Teachers Profile'),
                        sac.StepsItem(
            ...
            if transcript:
                if assessment_type == "Transcribing No Assessment":
                    ...
                    if subject and topic and language:
                        assessment_prompt(
                            transcript, assessment_type, subject, topic, language)
                    else:
                        st.warning(
