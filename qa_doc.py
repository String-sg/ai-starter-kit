...
def delete_files_from_db(file_names, username, profile):
	conn = sqlite3.connect(st.session_state.db_path)
	cursor = conn.cursor()
...
def docs_uploader():
	...
	if files:
		...
		if build == 'Delete Selected Files':
			delete_files_from_db(selected_files_to_delete, st.session_state.user["username"], st.session_state.user["profile"])
...
def create_lancedb_table(meta, table_name):
	lancedb_path = os.path.join(st.session_state.db_working, "lancedb")
	# LanceDB connection
...
def delete_vector_instances_from_db(selected_keys, current_username, profile):
	conn = sqlite3.connect(st.session_state.db_path)
	cursor = conn.cursor()
...
def dbs_deleter():
	...
	if vector_instance_keys:
		...
		if build == 'Delete Selected Vector Instances':
			delete_vector_instances_from_db(selected_keys_to_delete, st.session_state.user["username"], st.session_state.user["profile"])
