...
def lesson_commentator():
    ...
    subject = st.selectbox("Choose a Subject", SUBJECTS_SINGAPORE)
    level = st.text_input("Choose a level", EDUCATION_LEVELS)
    ...
    try:
        if prompt:
            ...
            if st.session_state.vs:
                ...
                st.warning(
                    "There is no knowledge base linked to this generative AI form")
                reference_prompt = ""
            full_response = ""
            ...
            # st.write(num_tokens)
            insert_into_data_table(now.strftime(
                "%d/%m/%Y %H:%M:%S"),  full_response, prompt, num_tokens, feedback_value)
